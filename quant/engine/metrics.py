...
def summary(
    ...
    return {
        ...
        "AnnVol": float(vol),
        "MaxDD": float(mdd_abs),
        "MaxDDPct": float(mdd_pct),
        "DD_Start": str(dd_start) if dd_start is not None else None,
        "DD_End": str(dd_end) if dd_end is not None else None,
        "Days>0": wins,
        "Days<0": losses,
        "WinRate(Day)": float(win_rate),
        "AvgWin(Day)": float(avg_win),
        "AvgLoss(Day)": float(avg_loss),
    }